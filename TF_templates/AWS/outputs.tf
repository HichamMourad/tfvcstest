output "instance_id" {
  ...
  value       = aws_instance.rhel_server[count.index].id
}

output "instance_public_ip" {
  description = "Public IP address of the EC2 instance"
  value       = aws_instance.rhel_server[count.index].public_ip
}

output "instance_public_dns" {
  description = "The Public DNS of the instance."
  value       = "${formatlist("%v", aws_instance.rhel_server[count.index].*.public_dns)}"
}
